...
function brandsCountArr(arr) {
  //create array with brands and count
  let brandsArray = [];
  for (let i = 0; i < arr.length; i++) {
    let phones = arr[i].phones;

    for (let j = 0; j < phones.length; j++) {
      let brandExists = brandsArray.filter((obj) => {
        return obj.brand === phones[j];
      });

      if (brandExists.length === 0) {
        brandsArray.push({ brand: phones[j], count: 0 });
      }
    }
  }
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    //loop through all phones and add brand count
    for (let j = 0; j < phones.length; j++) {
      let phone = phones[j];
...
function mostTotalGadgets(arr) {
  //create array with gadgets and gadget count
  let gadgetsArray = [];
  for (let i = 0; i < arr.length; i++) {
    let developerName = arr[i].name;
    gadgetsArray.push({
